...
$signupDisplay = 'SELECT * FROM `__global`.`'.$theseRegistrants.'` WHERE dateID = ' .$selectedDate. ' ORDER BY timeID ASC;';
...
echo "<tr><td><form name='deleteStudent' action='delete_student2.php'>";
...
while($signupRow = mysql_fetch_array($signupResult)){
	...
	$timeDisplay = 'SELECT slot_time FROM `__global`.`'.$theseTimes.'` WHERE date = ' .$selectedDate. ' AND id = '.$signupRow['timeID'].';';
