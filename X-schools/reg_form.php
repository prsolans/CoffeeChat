...
function ajaxTime() {
  $.ajax({
	type: 'GET',
	url: 'time.php',
	data: 'school=<?php echo $thisSchool; ?>&date='+document.getElementById("date").value+'&date2='+document.getElementById("date").name,
...
<table>
  <tr>
    ...
    <td><?php	 	 		 		 	 	 		 		 		 		 		 	 
$datesQuery = 'SELECT id, slot_date FROM `'.$database.'`.`EventDate` WHERE school = '.$thisSchool.' AND active = 0 ORDER BY slot_date;';
